...
//void execute( long id)
{
	...
	stream << "creating thread: " << myid << " ..."<< endl;
	...
	time(&seed);
    irand((long)(seed+myid));

	Big order=pfc.order();
	for(int i=0; i< NUM_TIMES; i++){
		t[i] = 0;
	}
	stream << "Starting pairing and multiplication benchmark, n=" << n << " iterations" << endl;
	for(int i=0; i<n; i++){
		pfc.random(g1[i]);
		pfc.random(g2[i]);
		pfc.random(l1[i]);
	...
	for(int i=0; i<n; i++){
		...
		if(j == n) j=0;
		if(ML[i] != (L[i]*L[j])){
			stream << "multi_pairing ML[i] != (L[i]*L[j]) not working as expected!" << endl;
			exit(0);
		}
	}
	...
	for(int i=0; i<n; i++){
		...
		if(j == n) j=0;
		if(MR[i] != (R[i]*R[j])){
			stream << "multi_pairing MR[i] != (R[i]*R[j]) not working as expected!" << endl;
			exit(0);
		}
	}
	stream << "Starting 3x multi_pair " << endl;
	...
	//*********multiplication****************
	stream << "Starting group multiplication benchmark" << endl;
	...
	//*********exponentiation****************
	stream << "Starting exponentiation benchmark" << endl;
	...
	cout << stream.str() << ", thread " << myid << endl;
}
...
int main(int argc, char* argv[])
{
   ...
   pthread_attr_init(&attributes);
    ...
    pthread_t tids[k];
    for(int i=0;i<k;i++){
		pthread_create(&tids[i],&attributes,execute,(void *)id++);
    }
    for(int i=0;i<k;i++){
    	pthread_join(tids[i],&status);
    }
    double t = (clock()-start)/(double)CLOCKS_PER_SEC;
    cout << "Total time (ms) to run " << k << "threads = " << t <<endl;
    for(int i=0;i<k;i++){
